      ...
      <section className='bg-linear-to-b from-purple-400 to-purple-600 p-5'>
        ...
        <div className='flex flex-row flex-wrap justify-evenly my-5 gap-5'>
          {bakeryData.filter(item => item.isBestSeller === true).slice(0, 4).map(({ title, imageUrl, price }, index) => (
            <div key={index} className="flex flex-col justify-between bg-gray-300 w-[65%] md:w-60 shadow-black rounded-lg ml-5 hover:shadow-2xl active:shadow-2xl hover:cursor-pointer relative">
      ...
      <section className='bg-linear-to-b from-purple-500 to-purple-700'>
        ...
        <div className='flex flex-row flex-wrap justify-center items-center gap-20 p-10 w-full h-full '>
            {[
              ...
              <section key={flav}>
                <div className='group relative w-50 h-50 rounded-xl overflow-hidden  hover:cursor-pointer hover:scale-105 active:scale-105'>
                  <img src={url} alt={url} className='h-full w-full absolute object-cover brightness-40 group-hover:brightness-60 group-active:brightness-60' />
